...
#ou_mu_r = log(0.001);
ou_lambda_r = 1 / 14;
WNV_model <- function(sirWNV_ensemble, Vm_t, r_t, f_t, par_input, week) {
  ...
  # Parameters
  par <- c(Tmh = par_input[1], psi = par_input[2])
  
  # Get temperature and precipitation for this week
  week_correct <- week - 1
  ...
  for (i in 1:7) {
    ...
    dSm <- (Vm_t[i]  - temp_growth1 - mu * Sm) * dt
...
num_iterations <- 46  # Number of iterations


results <- list()  # Store output
...
# Full ensemble storage (state variables + parameters)
save_ensemble_full_global <- array(0, dim = c(10, N, total_time_points))
static2_global <- array(0, dim = c(2, N, total_time_points))
static_global <- matrix(0, nrow = 2, ncol = N)
# Diagnostics
...
ensemble <- rbind(Sm_init_particles, Im_init_particles,
                  ...
                  log_Vm_t_particles,log_r_t_particles, log_f_t_particles, 
                  Tmh_particles, psi_particles)

param_names <- c("Tmh", "psi")
...
for (iteration in 1:num_iterations) {
  ...
  for (obs_index in obs_indices) { 
    ...
    # Forecast step
    for (i in 1:N) {
      #Vm <- as.numeric(exp(ensemble[6, i]))  # Vm for each ensemble member
      #r <- as.numeric(exp(ensemble[7, i]))
      #f <- as.numeric(exp(ensemble[8, i]))
      #par <- as.numeric(ensemble[9:10, i])
      ...
      sirWNV_output <- WNV_model(ensemble[1:5, i, drop = FALSE], 
                                 Vm_t = Vm_vec, r_t = rt_vec, f_t = ft_vec, par_input = as.numeric(ensemble[9:10, i]), week = week)
    ...
    # Vm_t_particles = OUproc_func(exp(ensemble[6, ]),
    #                              mu = ou_mu_vm,
    #                              lambda = ou_lambda_vm,
    #                              sigma = ou_sigma_vm)
    # log_Vm_t_particles = log(Vm_t_particles)
    # 
    # ensemble[6, ] <- log_Vm_t_particles
    # 
    # #JOE:
    # # Time-varying sigma for log(r)
    # 
    # if (obs_index<as.numeric(obs_year[as.character(Year)])) {#adjust when you observe data earlier
    #   ou_sigma_r <- 0.002
    #   ou_mu_r = log(0.001)
    # } else {
    #   ou_sigma_r <- 1.50
    ...
    # r_t_particles = OUproc_func(exp(ensemble[7, ]),
    #                             mu = ou_mu_r,
    #                             lambda = ou_lambda_r,
    #                             sigma = ou_sigma_r)
    # log_r_t_particles = log(r_t_particles)
    # ensemble[7, ] <- log_r_t_particles
    # f_t_particles = OUproc_func(exp(ensemble[8, ]),
    #                             mu = ou_mu_r,
    #                             lambda = ou_lambda_r,
    #                             sigma = ou_sigma_r)
    # log_f_t_particles = log(f_t_particles)
    # ensemble[8, ] <- log_f_t_particles
      ...
      #   Pxy %*% solve(Py + R_temp)
      # }, error = function(e) {
      #   message("Using pseudo-inverse due to singular matrix at time ", t_index)
      #   Pxy %*% ginv(Py + R_temp)
      # })
      ...
      for (i in 1:N) {
        ...
        #ensemble[, i] <- ensemble[, i] + K %*% (innovation + obs_noise - (obs_ensemble[, i] - obs_mean))
        ensemble[, i] <- ensemble[, i] + K %*% (observation - obs_ensemble[, i] + obs_noise)
      }
      
      for(k in 1:5){ensemble[k, ] = pmax(ensemble[k, ], 1e-6)}
      for(k in 9:10){ensemble[k, ] = pmax(ensemble[k, ], 1e-6)}
    ...
    static_global[,] <- ensemble[9:10, ]
    save_ensemble_full_global[, , obs_index] <- ensemble
    static2_global[, , obs_index] <- ensemble[9:10, ]
  }
  
  if (iteration %in% c(46)) {
    saveRDS(save_ensemble_full_global, file = paste0("save_ensemble_full_global_Mosq+Human+NoClimate_", Year, ".rds"))
  ...
  if (iteration %in% c(46)) {
    ggsave(
      paste0("vm_daily_OU_Mosq+Human+NoWeather_", Year, ".png"),
      plot   = p_vm_daily,
      width  = 10, height = 5, dpi = 300
    )
    ggsave(
      paste0("rt_daily_OU_Mosq+Human+NoWeather_", Year, ".png"),
      plot   = p_rt_daily,
      width  = 10, height = 5, dpi = 300
    )
    ggsave(
      paste0("ft_daily_OU_Mosq+Human+NoWeather_", Year, ".png"),
      plot   = p_ft_daily,
      width  = 10, height = 5, dpi = 300
    )
    ...
    ggsave(
      paste0("vm_rt_ft_daily_OU_Mosq+Human+NoWeather_", Year, ".png"),
  ...
  if (iteration %in% c(46)) {
    ggsave(paste0("A1_iteration0_", iteration, "_", Year, ".png"), plot = A1, width = 10, height = 8, dpi = 300)
    ggsave(paste0("A2_iteration0_", iteration, "_", Year, ".png"), plot = A2, width = 10, height = 8, dpi = 300)
    ggsave(paste0("A3_iteration0_", iteration, "_", Year, ".png"), plot = A3, width = 10, height = 8, dpi = 300)
    ggsave(paste0("A4_iteration0_", iteration, "_", Year, ".png"), plot = A4, width = 10, height = 8, dpi = 300)
    ggsave(paste0("A5_iteration0_", iteration, "_", Year, ".png"), plot = A5, width = 10, height = 8, dpi = 300)
    ggsave(paste0("A6_iteration0_", iteration, "_", Year, ".png"), plot = A6, width = 10, height = 8, dpi = 300)
    ggsave(paste0("A7_iteration0_", iteration, "_", Year, ".png"), plot = S7, width = 10, height = 8, dpi = 300)
    ggsave(paste0("A8_iteration0_", iteration, "_", Year, ".png"), plot = S8, width = 10, height = 8, dpi = 300)
    ggsave(paste0("A9_iteration0_", iteration, "_", Year, ".png"), plot = S9, width = 10, height = 8, dpi = 300)
    #ggsave(paste0("A35_iteration0_", iteration, ".png"), plot = p1, width = 10, height = 8, dpi = 300)
    ggsave(paste0("A36_iteration0_", iteration, "_", Year, ".png"), plot = p2, width = 10, height = 8, dpi = 300)
    # --- Parameter histograms ---
    # png(paste0("Param_Hist_iteration0_", iteration, ".png"), width = 1200, height = 800)
    # par(mfrow = c(2, 1))  
    # for (i in 1:2) {
  ...
  selected_values3 <- save_ensemble_full_global[9:10, random_indices, 1:final_obs_count]
  ...
  for (i in 1:n_forecast_sample) {
    ...
    # ft_pred = vector("numeric", length = 2)
    ...
    #                          lambda = ou_lambda_r,
    #                          sigma = ou_sigma_r)
    # ft_pred[1] = OUproc_func(tail(fitted_vals_ft, 1),
    #                          mu = ou_mu_r,
    #                          lambda = ou_lambda_r,
    #                          sigma = ou_sigma_r)
    # ft_pred[2] = OUproc_func(ft_pred[1],
    #                          mu = ou_mu_r,
    #                          lambda = ou_lambda_r,
    #                          sigma = ou_sigma_r)
...
#results <- readRDS(paste0("results_Mosq+Human+NoClimate_", Year, ".rds"))
#####################################
...
saveRDS(wis_all, file = paste0("wis_all_Mosq+Human+NoClimate_", Year, ".rds"))
#wis_all <- readRDS(paste0("wis_all_Mosq+Human+NoClimate_", Year, ".rds")) 
# 
# # 2. Summarize WIS
...
# 
# # 6. Save results
#write.csv(wis_summary, "wis_summary.csv", row.names = FALSE)
#write.csv(normalized_wis, "normalized_wis.csv", row.names = FALSE)
# 
# # Example: Calculate WIS for a single forecast manually
# single_forecast <- results[[1]]$total_abundance_q_1
# single_actual <- X_obs1[6]  # Actual value for 1-week ahead from iteration 1
# # 
# pred_dist <- quantile_pred(
#   matrix(as.numeric(single_forecast), nrow = 1),
#   c(0.01, 0.025, 0.05, 0.1, 0.15, 0.2, 0.25, 0.3, 0.35, 0.4, 
#     0.45, 0.5, 0.55, 0.6, 0.65, 0.7, 0.75, 0.8, 0.85, 0.9, 
#     0.95, 0.975, 0.99)
# )
# # 
# wis_single <- weighted_interval_score(
#   x = pred_dist,
#   actual = single_actual,
#   na_handling = "impute"
# )
# 
# print(paste("WIS for single forecast:", wis_single))
...
ggsave(paste0("Q1_iteration0_", iteration, "_", Year, ".png"), plot = Q1, width = 10, height = 8, dpi = 300)
ggsave(paste0("Q2_iteration0_", iteration, "_", Year, ".png"), plot = Q2, width = 10, height = 8, dpi = 300)
ggsave(paste0("Q3_iteration0_", iteration, "_", Year, ".png"), plot = Q3, width = 10, height = 8, dpi = 300)
ggsave(paste0("Q4_iteration0_", iteration, "_", Year, ".png"), plot = Q4, width = 10, height = 8, dpi = 300)
ggsave(paste0("Q5_iteration0_", iteration, "_", Year, ".png"), plot = Q5, width = 10, height = 8, dpi = 300)
ggsave(paste0("Q6_iteration0_", iteration, "_", Year, ".png"), plot = Q6, width = 10, height = 8, dpi = 300)
saveRDS(results, file = paste0("results_Mosq+Human+NoClimate_", Year, ".rds"))
...
plot_flusight_style <- function(results,
  ...
  p <- p +
    ...
    labs(
      ...
      subtitle = "Fan chart: 2-week ahead forecasts at each forecast origin",
      x = "Date",
      y = "Count"
    ) +
...
p_abundance <- plot_flusight_style(
  ...
  target               = "total_abundance",
  model_name           = "Mosq+Human+NoClimate",
...
# Save plot 
ggsave("f_abundance_Mosq+Human+NoClimate.png", p_abundance, width = 11, height = 6, dpi = 300)
ggsave("f_infectious_per_1000_Mosq+Human+NoClimate.png", p_infected, width = 11, height = 6, dpi = 300)
ggsave("f_Humancases_Mosq+Human+NoClimate.png", p_cases, width = 11, height = 6, dpi = 300)

#used to calculate boom years for abundance

library(dplyr)

# ════════════════════════════════════════════════════════════════
# Identify "boom" years based on peak total mosquito abundance
# ════════════════════════════════════════════════════════════════

years_to_run <- c(2006:2019, 2021)

abundance_by_year <- data.frame(
  year           = years_to_run,
  peak_abundance = NA_real_
)

for (Year in years_to_run) {
  
  cat("\nLoading abundance data for year", Year, "...\n")
  
  result <- tryCatch({
    
    # ──────────────────────────────────────────────────────────────
    # YEAR-SPECIFIC ABUNDANCE LOADING (mirrors your original loader)
    # ──────────────────────────────────────────────────────────────
    if (Year == 2021) {
      ...
      stop("Year must be one of: 2006-2020 or 2021")
    }
    
    # ── Capture peak abundance for this year ──
    max(X_obs, na.rm = TRUE)
    
  }, error = function(e) {
    cat("  ERROR loading year", Year, ":", conditionMessage(e), "\n")
    NA_real_
  })
  
  abundance_by_year$peak_abundance[abundance_by_year$year == Year] <- result
}

# ════════════════════════════════════════════════════════════════
# Determine boom-year threshold (top quartile) and report
# ════════════════════════════════════════════════════════════════

threshold <- quantile(abundance_by_year$peak_abundance, 0.75, na.rm = TRUE)

boom_years <- abundance_by_year %>%
  filter(peak_abundance >= threshold) %>%
  arrange(desc(peak_abundance))

cat("\n──────────────────────────────────────────────\n")
cat("All years, ranked by peak total abundance:\n")
print(abundance_by_year %>% arrange(desc(peak_abundance)))

cat("\n75th percentile threshold:", round(threshold, 1), "\n")
cat("\nBoom years (top quartile):\n")
print(boom_years)

cat("\n", nrow(boom_years), "of", nrow(abundance_by_year),
    "years qualify as 'boom' years.\n")

library(dplyr)

# ════════════════════════════════════════════════════════════════
# Compute monsoon-season (June-September) cumulative precipitation
# per year, to check whether "boom" abundance years coincide with
# unusually high monsoon precipitation
# ════════════════════════════════════════════════════════════════

years_to_run <- c(2006:2019, 2021)

precip_by_year <- data.frame(
  year           = years_to_run,
  monsoon_precip = NA_real_,
  n_days         = NA_integer_
)

for (Year in years_to_run) {
  
  cat("\nLoading precipitation data for year", Year, "...\n")
  ...
  result <- tryCatch({
    ...
    if (Year == 2021) {
      ...
      column_name <- "rowMeans.combined_p..na.rm...TRUE."
      inputP_i <- as.numeric(P_temp[[column_name]])
      inputP_i <- inputP_i[731:1096]  # 366 days, leap year
    ...
    } else if (Year %in% 2006:2012) {
      
      P_temp <- read.csv("./county_PRCP_2006-2012.csv")
      P_temp <- P_temp %>% filter(Year == .env$Year)   # .env$Year avoids clashing with the "Year" column
      inputP_i <- as.numeric(P_temp$rowMeans.combined_p..na.rm...TRUE.)
    ...
    # ── Build a daily date vector matching inputP_i's actual length ──
    dates_year <- seq(as.Date(paste0(Year, "-01-01")),
                      by = "day", length.out = length(inputP_i))
    
    # ── Sum precipitation across June-September (monsoon season) ──
    month_num   <- as.numeric(format(dates_year, "%m"))
    monsoon_idx <- which(month_num %in% 6:9)
    
    list(
      monsoon_precip = sum(inputP_i[monsoon_idx], na.rm = TRUE),
      n_days         = length(monsoon_idx)
    )
    
  }, error = function(e) {
    cat("  ERROR loading year", Year, ":", conditionMessage(e), "\n")
    list(monsoon_precip = NA_real_, n_days = NA_integer_)
  })
  
  precip_by_year$monsoon_precip[precip_by_year$year == Year] <- result$monsoon_precip
  precip_by_year$n_days[precip_by_year$year == Year]         <- result$n_days
}

# ════════════════════════════════════════════════════════════════
# Rank years by monsoon precipitation and compare to abundance boom years
# ════════════════════════════════════════════════════════════════

boom_year_list <- c(2014, 2018, 2019, 2021)   # from the abundance analysis

comparison <- precip_by_year %>%
  mutate(
    precip_rank  = rank(-monsoon_precip, na.last = "keep"),
    is_boom_year = year %in% boom_year_list
  ) %>%
  arrange(precip_rank)

cat("\n──────────────────────────────────────────────\n")
cat("All years, ranked by monsoon-season (Jun-Sep) cumulative precipitation:\n")
print(comparison)

n_years <- sum(!is.na(precip_by_year$monsoon_precip))
top_half_precip_years <- comparison %>%
  filter(precip_rank <= n_years / 2) %>%
  pull(year)

cat("\nTop half of years by monsoon precipitation:",
    paste(sort(top_half_precip_years), collapse = ", "), "\n")
cat("Boom years (abundance):", paste(sort(boom_year_list), collapse = ", "), "\n")
cat("Boom years that are ALSO top-half for monsoon precipitation:",
    paste(sort(intersect(boom_year_list, top_half_precip_years)), collapse = ", "), "\n")
cat("Boom years that are NOT top-half for monsoon precipitation:",
    paste(sort(setdiff(boom_year_list, top_half_precip_years)), collapse = ", "), "\n")

library(dplyr)

# ════════════════════════════════════════════════════════════════
# Rank years by total annual human WNV case burden
# (final value of the cumulative case count = total cases that year)
# ════════════════════════════════════════════════════════════════

years_to_run <- c(2006:2019, 2021)

cases_by_year <- data.frame(
  year        = years_to_run,
...
for (Year in years_to_run) {
  ...
  cat("\nLoading human case data for year", Year, "...\n")
  ...
  result <- tryCatch({
    ...
    } else if (Year %in% c(2007:2011, 2013:2017)) {
      
      WNV_humans_summary3 <- read.csv("./WNV_humans_summary3_2006-2017.csv")
      X3_obs <- WNV_humans_summary3 %>% filter(YEAR == .env$Year)
      X3_obs <- X3_obs[-1, ]
      X3_obs <- as.numeric(X3_obs$cases)
      X3_obs <- cumsum(X3_obs)
    ...
    max(X3_obs, na.rm = TRUE)   # total cases for the year = final cumulative value
  ...
  cases_by_year$total_cases[cases_by_year$year == Year] <- result
}

# ════════════════════════════════════════════════════════════════
# Rank years and check the manuscript's claim
# ════════════════════════════════════════════════════════════════

ranked_cases <- cases_by_year %>% arrange(desc(total_cases))

cat("\n──────────────────────────────────────────────\n")
cat("All years, ranked by total annual human WNV cases:\n")
print(ranked_cases)

n_years <- sum(!is.na(cases_by_year$total_cases))
cat("\nTop 4 years (highest burden):\n")
print(head(ranked_cases, 4))
cat("\nBottom 4 years (lowest burden):\n")
print(tail(ranked_cases, 4))

claimed_high <- c(2012, 2021)
claimed_low  <- c(2009, 2015)

ranked_with_rank <- ranked_cases %>% mutate(rank = row_number())

cat("\nClaimed high-burden years and their actual rank (out of", n_years, "):\n")
print(ranked_with_rank %>% filter(year %in% claimed_high))

cat("\nClaimed low-burden years and their actual rank (out of", n_years, "):\n")
print(ranked_with_rank %>% filter(year %in% claimed_low))
...
# ── Load raw (non-relative) forecast WIS for all individual models ────────────
raw_wis_all <- map_dfr(names(model_files), function(model_name) {
  map_dfr(years, function(yr) {
    ...
    bind_rows(
      ...
      mutate(year = yr, model = model_name) %>%
      select(year, model, horizon, target, WIS)
  })
})

# ── Median raw WIS per year, per target, across all 4 models ─────────────────
raw_wis_by_year <- raw_wis_all %>%
  group_by(year, target) %>%
  summarise(
    median_absolute_WIS = median(WIS[is.finite(WIS)], na.rm = TRUE),
    .groups = "drop"
  ) %>%
  arrange(target, desc(median_absolute_WIS))

print(raw_wis_by_year, n = 50)

# ── Cross-reference against verified case-burden ranking ─────────────────────
high_burden_years <- c(2021, 2019, 2010)   # from confirmed case-count ranking
low_burden_years  <- c(2009, 2018, 2011)

raw_wis_by_year %>%
  filter(target == "Human cases") %>%
  mutate(
    burden_group = case_when(
      year %in% high_burden_years ~ "high-burden",
      year %in% low_burden_years  ~ "low-burden",
      TRUE ~ "other"
    )
  ) %>%
...
# ── 1. Central tendency: median relative WIS by model x target ───────────────
# (averaged across both horizons within each year first, then across years)
central_tendency <- summary_stats_log %>%
  group_by(model, target) %>%
  summarise(median_relWIS = median(median_rel_wis, na.rm = TRUE), .groups = "drop") %>%
  filter(model %in% c("Ensemble_model_1", "Ensemble_model_2", "Ensemble_model_4",
                      individual_models)) %>%
  arrange(target, median_relWIS)

print(central_tendency, n = 50)

# ── 2. Year-to-year variability: SD and IQR of yearly relative WIS,
#      by model x target, after averaging across horizons within each year ──
yearly_avg <- summary_stats_log %>%
  group_by(year, model, target) %>%
  summarise(yearly_relWIS = mean(median_rel_wis, na.rm = TRUE), .groups = "drop")

# Identify the best individual model per target (lowest median) to compare against
best_individual_per_target <- central_tendency %>%
  filter(model %in% individual_models) %>%
  group_by(target) %>%
  slice_min(median_relWIS, n = 1) %>%
  select(target, best_individual_model = model)

print(best_individual_per_target)

variability <- yearly_avg %>%
  group_by(model, target) %>%
  summarise(
    sd_relWIS  = sd(yearly_relWIS, na.rm = TRUE),
    iqr_relWIS = IQR(yearly_relWIS, na.rm = TRUE),
    .groups = "drop"
  ) %>%
  filter(model %in% c("Ensemble_model_1", "Ensemble_model_2", "Ensemble_model_4")) %>%
  left_join(best_individual_per_target, by = "target") %>%
  arrange(target, model)

print(variability, n = 50)

# ── 3. Direct comparison: ensemble variability vs. its target's best individual model ──
best_individual_variability <- yearly_avg %>%
  inner_join(best_individual_per_target,
             by = c("target", "model" = "best_individual_model")) %>%
  ...
  rename(best_individual_sd = sd_relWIS, best_individual_iqr = iqr_relWIS)

print(best_individual_variability)

library(dplyr)

all_models <- c("FullModel", "FullModel_NoWeather",
                "Mosq+Human+Weather", "Mosq+Human+NoWeather",
...
ensemble_models <- c("Ensemble_model_1", "Ensemble_model_2", "Ensemble_model_3",
                     "Ensemble_model_4", "Ensemble_model_5")

# ════════════════════════════════════════════════════════════════
# 1. Central tendency: median relative WIS, ALL 9 models x 3 targets
# ════════════════════════════════════════════════════════════════
central_tendency_full <- summary_stats_log %>%
  filter(model %in% all_models) %>%
  group_by(model, target) %>%
  summarise(median_relWIS = median(median_rel_wis, na.rm = TRUE), .groups = "drop") %>%
  arrange(target, median_relWIS)

print(central_tendency_full, n = 50)

# ── Rank within target, to identify the single best model per target ─────────
ranked_full <- central_tendency_full %>%
  group_by(target) %>%
  mutate(rank = rank(median_relWIS)) %>%
  arrange(target, rank)

cat("\n── Best model per target (rank 1 of 9) ──\n")
print(ranked_full %>% filter(rank == 1))

cat("\n── Where does Ensemble Model 4 rank for each target? ──\n")
print(ranked_full %>% filter(model == "Ensemble_model_4"))

# ════════════════════════════════════════════════════════════════
# 2. Year-to-year variability: SD and IQR, ALL 9 models x 3 targets
#    (averaged across horizons within each year first)
# ════════════════════════════════════════════════════════════════
yearly_avg_full <- summary_stats_log %>%
  filter(model %in% all_models) %>%
  group_by(year, model, target) %>%
  summarise(yearly_relWIS = mean(median_rel_wis, na.rm = TRUE), .groups = "drop")

variability_full <- yearly_avg_full %>%
  group_by(model, target) %>%
  summarise(
    sd_relWIS  = sd(yearly_relWIS, na.rm = TRUE),
    iqr_relWIS = IQR(yearly_relWIS, na.rm = TRUE),
  ...
  arrange(target, sd_relWIS)

print(variability_full, n = 50)

# ════════════════════════════════════════════════════════════════
# 3. Combined table: central tendency + variability + model type,
#    side by side, sorted for easy scanning per target
# ════════════════════════════════════════════════════════════════
combined_full <- central_tendency_full %>%
  left_join(variability_full, by = c("model", "target")) %>%
  mutate(
    model_type = if_else(model %in% individual_models, "individual", "ensemble")
  ) %>%
  group_by(target) %>%
  arrange(target, median_relWIS) %>%
  mutate(rank = row_number()) %>%
  ungroup() %>%
  select(target, rank, model, model_type, median_relWIS, sd_relWIS, iqr_relWIS)

cat("\n── Full combined table, all 9 models x 3 targets ──\n")
print(combined_full, n = 50)

# ════════════════════════════════════════════════════════════════
# 4. Specifically: how do Ensemble 3 and 5 compare to Ensemble 4?
#    (the two we hadn't checked yet)
# ════════════════════════════════════════════════════════════════
cat("\n── Ensemble 3 and 5 vs Ensemble 4, all targets ──\n")
print(
  combined_full %>%
    filter(model %in% c("Ensemble_model_3", "Ensemble_model_4", "Ensemble_model_5")) %>%
    arrange(target, rank)
)
library(dplyr)
library(purrr)

# ── Check direction of abundance forecast error for 2019 specifically ─────────
# Load the Full Model abundance forecasts for 2019
# (adjust file path/name to match your actual saved forecast object)

fpath <- "wis_all_FullModel_2019.rds"
lis   <- readRDS(fpath)

# Look at both horizons
abundance_2019 <- bind_rows(
  lis$abundance_1wk %>% mutate(horizon = 1),
  lis$abundance_2wk %>% mutate(horizon = 2)
) %>%
  mutate(
    error_direction = case_when(
      median_forecast < actual ~ "under-prediction",
      median_forecast > actual ~ "over-prediction",
      TRUE                     ~ "exact"
    ),
    abs_error = abs(median_forecast - actual)
  )

# Overall direction summary
cat("Direction of abundance forecast error for Full Model 2019:\n")
print(
  abundance_2019 %>%
    group_by(horizon, error_direction) %>%
    summarise(n = n(), mean_abs_error = mean(abs_error, na.rm = TRUE), .groups = "drop") %>%
    arrange(horizon, desc(n))
)

# Median forecast vs actual across the season
cat("\nMedian forecast vs actual, by horizon:\n")
print(
  abundance_2019 %>%
    group_by(horizon) %>%
    summarise(
      median_actual   = median(actual,          na.rm = TRUE),
      median_forecast = median(median_forecast, na.rm = TRUE),
      pct_under       = mean(median_forecast < actual, na.rm = TRUE) * 100,
      .groups = "drop"
    )
)

# Also check for all 4 individual models to see if the pattern is consistent
individual_models <- c("FullModel", "FullModel_NoWeather",
                       "Mosq+Human+Weather", "Mosq+Human+NoWeather")

model_files <- list(
  "Full Model"           = "wis_all_FullModel_",
  "FullModel_NoWeather"  = "wis_all_FullModel_NoClimate_",
  "Mosq+Human+Weather"   = "wis_all_Mosq+Human+Climate_",
  "Mosq+Human+NoWeather" = "wis_all_Mosq+Human+NoClimate_"
)

direction_all_models_2019 <- map_dfr(names(model_files), function(m) {
  fpath <- paste0(model_files[[m]], "2019.rds")
...
cat("\nDirection summary for all 4 individual models, 2019 abundance:\n")
print(direction_all_models_2019)
dry_years <- c(2007, 2009, 2011)  # already checked 2019

direction_dry_years <- map_dfr(dry_years, function(yr) {
  map_dfr(names(model_files), function(m) {
    fpath <- paste0(model_files[[m]], yr, ".rds")
    if (!file.exists(fpath)) return(NULL)
    lis <- readRDS(fpath)
    bind_rows(
      lis$abundance_1wk %>% mutate(horizon = 1),
      ...
      mutate(model = m, year = yr)
  })
}) %>%
  group_by(year, model, horizon) %>%
  summarise(
    pct_under       = mean(median_forecast < actual, na.rm = TRUE) * 100,
    median_actual   = median(actual,          na.rm = TRUE),
    median_forecast = median(median_forecast, na.rm = TRUE),
  ...
  # Focus on weather-inclusive models only — those are the ones where
  # anthropogenic water would add something beyond current specification
  filter(model %in% c("Full Model", "Mosq+Human+Weather")) %>%
  arrange(year, horizon, model)

print(direction_dry_years, n = 30)
