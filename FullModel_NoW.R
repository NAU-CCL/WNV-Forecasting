...
# WNV Model Function
WNV_model <- function(sirWNV_ensemble, Vm_t, r_t, par_input, week) {
  ...
  # Parameters
  par <-  par <- c(Tmb = par_input[1], d = par_input[2], Tbm = par_input[3],
                   gamma = par_input[4], tau = par_input[5], Vb = par_input[6],
                   Tmh = par_input[7], psi = par_input[8])
  #print(par)
  ...
  #print(brim_temp)
  #temp_growth = …if(Nb == 0){ temp_growth = 0
  ...
  for (i in 1:7) {
    ...
    if (is.na(Nb) || Nb == 0) {
      ...
      temp_growth1 <- r_t[i] * par["Tmb"] * Ib * Sm / Nb
    }
    ...
    if (is.na(Nb) || Nb == 0) {
      ...
      temp_growth2 <- r_t[i] * par["Tbm"] * Im * Sb / Nb
    }
...
save_ensemble_full_global <- array(0, dim = c(18, N, total_time_points))
static2_global <- array(0, dim = c(8, N, total_time_points))
static_global <- matrix(0, nrow = 8, ncol = N)
# Diagnostics
...
#Vm_t_particles <- runif(N, min = 400.0001, max = 1427)
#Vm_t_particles <- runif(N, min = 3.0001, max = 10)
...
ensemble <- rbind(Sm_init_particles, Im_init_particles, Sb_init_particles, Ib_init_particles, Rb_init_particles,
                  Sh_init_particles, Eh_init_particles, Ih_init_particles,
                  log_Vm_t_particles,log_r_t_particles,
                  Tmb_particles, d_particles, Tbm_particles, gamma_particles, tau_particles, Vb_particles,
...
for (iteration in 1:num_iterations) {
  ...
  for (obs_index in obs_indices) { 
    ...
    # Forecast step
    for (i in 1:N) {
      #Vm <- as.numeric(exp(ensemble[9, i]))  # Vm for each ensemble member
     # r <- as.numeric(exp(ensemble[10, i]))
      #par <- as.numeric(ensemble[11:18, i])
      Vm_vec <- Vm_daily[i, ]      # length 7
      rt_vec <- rt_daily[i, ]      # length 7
      # Run the WNV model for each ensemble member
      sirWNV_output <- WNV_model(ensemble[1:8, i, drop = FALSE], 
                                 Vm_t = Vm_vec, r_t = rt_vec, par_input = as.numeric(ensemble[11:18, i]), week = week)
    ...
    static_global[,] <- ensemble[11:18, ]
    save_ensemble_full_global[, , obs_index] <- ensemble
    static2_global[, , obs_index] <- ensemble[11:18, ]
    }
    
  if (iteration %in% c(46)) {
    saveRDS(save_ensemble_full_global, file = paste0("save_ensemble_full_global_FullModel_NoClimate_", Year, ".rds"))
  ...
  if (iteration %in% c(46)) {
    ggsave(
      paste0("vm_daily_OU_FullModel_withoutweather_", Year, ".png"),
      plot   = p_vm_daily,
      width  = 10, height = 5, dpi = 300
    )
    ggsave(
      paste0("rt_daily_OU_FullModel_withoutweather_", Year, ".png"),
      plot   = p_rt_daily,
      width  = 10, height = 5, dpi = 300
    )
    ...
    ggsave(
      paste0("vm_rt_daily_OU_FullModel_withoutweather_", Year, ".png"),
...
S12 <- ggplot() +
  ...
  if (iteration %in% c(46)) {
    ggsave(paste0("A1_iteration0FULL0_", iteration, "_", Year, ".png"), plot = A1, width = 10, height = 8, dpi = 300)
    ggsave(paste0("A2_iteration0FULL0_", iteration, "_", Year, ".png"), plot = A2, width = 10, height = 8, dpi = 300)
    ggsave(paste0("A3_iteration0FULL0_", iteration, "_", Year, ".png"), plot = S10, width = 10, height = 8, dpi = 300)
    ggsave(paste0("A4_iteration0FULL0_", iteration, "_", Year, ".png"), plot = A4, width = 10, height = 8, dpi = 300)
    ggsave(paste0("A5_iteration0FULL0_", iteration, "_", Year, ".png"), plot = A5, width = 10, height = 8, dpi = 300)
    ggsave(paste0("A6_iteration0FULL0_", iteration, "_", Year, ".png"), plot = A6, width = 10, height = 8, dpi = 300)
    ggsave(paste0("A7_iteration0FULL0_", iteration, "_", Year, ".png"), plot = S7, width = 10, height = 8, dpi = 300)
    ggsave(paste0("A8_iteration0FULL0_", iteration, "_", Year, ".png"), plot = S8, width = 10, height = 8, dpi = 300)
    ggsave(paste0("A9_iteration0FULL0_", iteration, "_", Year, ".png"), plot = S9, width = 10, height = 8, dpi = 300)
    ggsave(paste0("A10_iteration0FULL0_", iteration, "_", Year, ".png"), plot = S11, width = 10, height = 8, dpi = 300)
    ggsave(paste0("A11_iteration0FULL0_", iteration, "_", Year, ".png"), plot = S12, width = 10, height = 8, dpi = 300)
    ...
    # --- Parameter histograms ---
     png(paste0("Param_Hist_iteration0FULL0_", iteration, "_", Year, ".png"), width = 1200, height = 800)
     par(mfrow = c(2, 4))
     for (i in 1:8) {
...
selected_values3 <- save_ensemble_full_global[11:18, random_indices, 1:final_obs_count]
  ...
  for (i in 1:n_forecast_sample) {
    ...
    static_medians <- as.numeric(selected_values3[,i,final_obs_count]) 
...
ggsave(paste0("Q1_iteration0FULL0_", iteration, "_", Year, ".png"), plot = Q1, width = 10, height = 8, dpi = 300)
ggsave(paste0("Q2_iteration0FULL0_", iteration, "_", Year, ".png"), plot = Q2, width = 10, height = 8, dpi = 300)
ggsave(paste0("Q3_iteration0FULL0_", iteration, "_", Year, ".png"), plot = Q3, width = 10, height = 8, dpi = 300)
ggsave(paste0("Q4_iteration0FULL0_", iteration, "_", Year, ".png"), plot = Q4, width = 10, height = 8, dpi = 300)
ggsave(paste0("Q5_iteration0FULL0_", iteration, "_", Year, ".png"), plot = Q5, width = 10, height = 8, dpi = 300)
ggsave(paste0("Q6_iteration0FULL0_", iteration, "_", Year, ".png"), plot = Q6, width = 10, height = 8, dpi = 300)
